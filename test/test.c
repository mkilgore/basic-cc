...
int run_tests(const char *mod_name, struct unit_test *tests, int test_count, int argc, char **argv)
{
    ...
    for (i = 0; i < test_count; i++) {
        ...
        printf("== Result: ");
        if (errs != 0)
            printf(TERM_COLOR_RED "FAIL -> %d" TERM_COLOR_RESET, errs);
        else
            printf(TERM_COLOR_GREEN "PASS" TERM_COLOR_RESET);
    ...
    if (error_count == 0)
        ...
        printf(TERM_COLOR_RED "FAIL -> %d " TERM_COLOR_RESET, error_count);
...
int assert_true(const char *arg, const char *func, int cond)
{
    ...
    if (cond)
        ...
        printf(TERM_COLOR_RED "FAIL" TERM_COLOR_RESET);
